...
pnpm preview     # preview the Vite production build on :39847
...
This repo can also run as a Tauri desktop app. The frontend is a Vite/React app;
local filesystem access, downloads, VLC launching, config, and OAuth token
exchange are handled by Tauri Rust commands.
...
`desktop:build` runs the Vite production build and packages the static frontend
into the Tauri app. No Node/Next sidecar is bundled.
...
AniList OAuth still needs a redirect URI that lands back on the app route. For
local development, register this redirect URI in AniList:
